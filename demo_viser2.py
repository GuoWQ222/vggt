...
def viser_wrapper(
    ...
    def visualize_frames(intrinsics: np.ndarray, extrinsics: np.ndarray, images_: np.ndarray) -> None:
        """
    ...
    visualize_frames(intrinsics_cam, cam_to_world, images)
...
def main():
    ...
    # image_names = sorted(image_names, key=lambda s: int(re.search(r"(\d+)", s).group(1)))
    image_names.sort()
